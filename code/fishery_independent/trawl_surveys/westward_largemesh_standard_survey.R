# notes ----
## summarise scallop catch from westward region largemesh survey for ghl packet
...
do.call(bind_rows, lapply(paste0("data/observer/catch/", 
                                 list.files("data/observer/catch/")), read_csv)) %>%
...
f_shp_prep("./data/maps/mgmt_units", "Scallop_KM_Districts_wgs84_ksh_ksw_new_bdry", fortify = T) %>%
  f_transform_crs(to = proj4string(raster::getData("GADM", country = c("USA"), level = 1, path = "./data/maps"))) %>%
  .[[1]] -> district_boundaries

## shapefile of largemesh survey stations
f_shp_prep("./data/maps/lm_survey_stations", "LMStationMasterAlbers", fortify = T) %>%
  f_transform_crs(to = proj4string(raster::getData("GADM", country = c("USA"), level = 1, path = "./data/maps"))) %>%
  .[[1]] -> wslm_stations
...
logbook %>%
  ...
  dplyr::select(Year, reg_area, district, lon, lat, data_type) -> tmp
  f_base_map+
  geom_polygon(data = wslm_stations, aes(x = long, y = lat, group = group), 
               color = "black", fill = cb_palette[5], alpha = 0.5)+
  coord_quickmap(xlim = c(-170, -150), ylim = c(52.5, 59))
...
## Area K
f_base_map+
  geom_polygon(data = district_boundaries, aes(x = long, y = lat, group = group), color = 1, fill = NA)+
...
ggsave("./figures/fishery_independent/2021/trawl_survey_vs_fishery_locations_area_K.png",
       plot = x,
...
ggsave("./figures/fishery_independent/2021/trawl_survey_vs_fishery_locations_area_O.png",
       plot = x,
       height = 7, width = 7, units = "in")
...
f_base_map+
  ...
  facet_wrap(~Year, ncol = 2) -> x
ggsave("./figures/fishery_independent/2021/trawl_survey_vs_fishery_locations_area_M.png",
       plot = x,
       height = 7, width = 7, units = "in")
...
ggsave("./figures/fishery_independent/2021/trawl_survey_O_size_comp.png", plot = x, height = 6, width = 4, units = "in")
...
wslm_specimen %>%
  ...
  facet_wrap(~Year, ncol = 1, scales = "free_y")+
  labs(x = "Shell Height (mm)", y = "Count") -> x
ggsave("./figures/fishery_independent/2021/trawl_survey_M_size_comp.png", plot = x, height = 6, width = 4, units = "in")
...
wslm_specimen %>%
  ...
  facet_wrap(~Year, ncol = 1, scales = "free_y")+
  labs(x = "Shell Height (mm)", y = "Count") -> x
ggsave("./figures/fishery_independent/2021/trawl_survey_KNE_size_comp.png", plot = x, height = 6, width = 4, units = "in")
### KSH
wslm_specimen %>%
  filter(district == "KSH",
         Year %in% 2016:2020) %>%
  ...
  facet_wrap(~Year, ncol = 1, scales = "free_y")+
  labs(x = "Shell Height (mm)", y = "Count") -> x
ggsave("./figures/fishery_independent/2021/trawl_survey_KSH_size_comp.png", plot = x, height = 6, width = 4, units = "in")
### KSW
wslm_specimen %>%
  filter(district == "KSW",
         Year %in% 2016:2020) %>%
  ...
  facet_wrap(~Year, ncol = 1, scales = "free_y")+
  labs(x = "Shell Height (mm)", y = "Count") -> x
ggsave("./figures/fishery_independent/2021/trawl_survey_KSW_size_comp.png", plot = x, height = 6, width = 4, units = "in")
### KSE
wslm_specimen %>%
  filter(district == "KSE",
         Year %in% 2016:2020) %>%
  ...
  facet_wrap(~Year, ncol = 1, scales = "free_y")+
  labs(x = "Shell Height (mm)", y = "Count") -> x
ggsave("./figures/fishery_independent/2021/trawl_survey_KSE_size_comp.png", plot = x, height = 6, width = 4, units = "in")
...
tmp %>%
  ...
  mutate(cpue_scaled = as.numeric(scale(cpue)),
         ...
         data_type = "Trawl Survey") %>%
  bind_rows(read_csv("./output/observer/2021/standardized_cpue_season_KNE.csv") %>%
              mutate(Year = as.numeric(substring(Season, 1, 4)),
...
ggsave("./figures/fishery_independent/2021/scaled_cpue_KNE.png", plot = x, height = 3, width = 6, units = "in")  
...
tmp %>%
  ...
  mutate(cpue_scaled = as.numeric(scale(cpue)),
         ...
         u95 = cpue_scaled + 1.96 * abs((cv * cpue_scaled)),
         data_type = "Trawl Survey") %>%
  bind_rows(read_csv("./output/observer/2021/standardized_cpue_season_KSH.csv") %>%
              mutate(Year = as.numeric(substring(Season, 1, 4)),
                     cpue_scaled = as.numeric(scale(std_cpue)),
                     data_type = "Fishery (Standardized)") %>%
              dplyr::select(Year, cpue_scaled, data_type) %>%
  ...
  labs(x = NULL, y = expression(paste("Scaled CPUE ( ", mu, " = 0, ", sigma, " = 1 )")), color = NULL)+
  scale_color_manual(values = cb_palette[c(3,4)]) -> x
ggsave("./figures/fishery_independent/2021/scaled_cpue_KSH.png", plot = x, height = 3, width = 6, units = "in")  
...
tmp %>%
  ...
  mutate(cpue_scaled = as.numeric(scale(cpue)),
         ...
         u95 = cpue_scaled + 1.96 * abs((cv * cpue_scaled)),
         data_type = "Trawl Survey") %>%
  bind_rows(read_csv("./output/observer/2021/standardized_cpue_season_KSW.csv") %>%
              mutate(Year = as.numeric(substring(Season, 1, 4)),
                     cpue_scaled = as.numeric(scale(std_cpue)),
                     data_type = "Fishery (Standardized)") %>%
              dplyr::select(Year, cpue_scaled, data_type) %>%
  ...
  labs(x = NULL, y = expression(paste("Scaled CPUE ( ", mu, " = 0, ", sigma, " = 1 )")), color = NULL)+
  scale_color_manual(values = cb_palette[c(3,4)]) -> x
ggsave("./figures/fishery_independent/2021/scaled_cpue_KSW.png", plot = x, height = 3, width = 6, units = "in")  
...
tmp %>%
  ...
  labs(x = NULL, y = expression(paste("Scaled CPUE ( ", mu, " = 0, ", sigma, " = 1 )")), color = NULL)+
  scale_color_manual(values = cb_palette[c(3,4)]) -> x
ggsave("./figures/fishery_independent/2021/scaled_cpue_KSE.png", plot = x, height = 3, width = 6, units = "in") 
...
tmp %>%
  ...
  mutate(cpue_scaled = as.numeric(scale(cpue)),
         ...
         u95 = cpue_scaled + 1.96 * abs((cv * cpue_scaled)),
         data_type = "Trawl Survey") %>%
  bind_rows(read_csv("./output/observer/2021/standardized_cpue_season_M.csv") %>%
              mutate(Year = as.numeric(substring(Season, 1, 4)),
                     cpue_scaled = as.numeric(scale(std_cpue)),
                     data_type = "Fishery (Standardized)") %>%
              dplyr::select(Year, cpue_scaled, data_type) %>%
  ...
  labs(x = NULL, y = expression(paste("Scaled CPUE ( ", mu, " = 0, ", sigma, " = 1 )")), color = NULL)+
  scale_color_manual(values = cb_palette[c(3,4)]) -> x
ggsave("./figures/fishery_independent/2021/scaled_cpue_M.png", plot = x, height = 3, width = 6, units = "in") 
...
f_base_map +
  ...
  coord_quickmap(xlim = c(-156.1, -150), ylim = c(55.7, 59))  -> x
ggsave("./figures/fishery_independent/2021/trawl_survey_cpue_map_area_K.png",
       plot = x,
...
f_base_map +
  ...
  geom_point(data = filter(tmp, reg_area == "M"), 
             ...
             alpha = 0.5, color = cb_palette[6])+
  labs(size = "Round Weight CPUE (kg / sq km)")+
  coord_quickmap(xlim = c(-165, -155), ylim = c(53.7, 57)) -> x
ggsave("./figures/fishery_independent/2021/trawl_survey_cpue_map_area_M.png",
       plot = x,
       height = 5, width = 5, units = "in")
...
f_base_map +
  geom_polygon(data = district_boundaries, aes(x = long, y = lat, group = group), color = 1, fill = NA)+
  geom_point(data = wslm_catch %>%
               filter(Year > 2016) %>%
               # add area swept, net width = 0.0122 km
               # add round weight cpue
               mutate(area_swept = distance_km * 0.0122,
                      rw_cpue = final_wt_kg / area_swept,
                      rw_cpue = ifelse(rw_cpue == 0, NA, rw_cpue)) %>%
               filter(reg_area == "O"), 
             aes(x = lon, y = lat, size = rw_cpue), 
             alpha = 0.5, color = cb_palette[6])+
  labs(size = "Round Weight CPUE (kg / sq km)")+
  coord_quickmap(xlim = c(-169, -164.2), ylim = c(52.5, 54.5)) -> x

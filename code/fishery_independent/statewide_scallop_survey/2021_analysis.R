...
## point estimates (rnd wt biomass in lbs)
  ...
  ungroup %>%
  # fix district  
  mutate(district = ifelse(grepl("KNE", bed_code), "KNE", district)) -> abundance_biomass
...
abundance_biomass %>%
  ...
  mutate(abund_plot_large = purrr::map(data, f_plot_abundance, group = 1, years = 2016:2021),
         ...
         biomass_plot_small = purrr::map(data, f_plot_biomass, group = 2, years = 2016:2021)) -> ab_plots
## save plots
...
mw_biomass %>%
  ## print output table
  write_csv("./output/fishery_independent/statewide_scallop_survey/2021/2021_mw_biomass_by_bed.csv")
...
ggsave("./figures/fishery_independent/2021/eki_2021_dredge_survey_size_comp.png", 
       plot = sh_plots$mt_biomass_plot[[2]], width = 5, height = 5, units = "in")
ggsave("./figures/fishery_independent/2021/yak_2021_dredge_survey_size_comp.png", 
       plot = sh_plots$mt_biomass_plot[[3]], width = 5, height = 6, units = "in")
...
shaw  %>%
  ...
  # plot
  ggplot()+
  geom_point(aes(x = shell_height, y = meat_wt, color = factor(year)), alpha = 0.3)+
...
ggsave("./figures/fishery_independent/2021/2021_dredge_survey_mw_sh.png", plot = x, 
...
shaw %>% 
  ...
  group_by(year, bed_code, district, samp_grp) %>%
  mutate(n_shucked = n()) %>%
  group_by(year, bed_code, district, mud_blister, samp_grp) %>%
...
write_csv(mud_blister, "./output/fishery_independent/statewide_scallop_survey/2021/2021_mud_blister.csv")
...
shaw %>% 
  ...
  group_by(year, bed_code, district, samp_grp) %>%
  mutate(n_shucked = n()) %>%
  group_by(year, bed_code, samp_grp, district, shell_worm) %>%
  summarise(perc = n() / mean(n_shucked) * 100) %>%
...
write_csv(shell_worm, "./output/fishery_independent/statewide_scallop_survey/2021/2021_shell_worm.csv")
...
shaw %>% 
  ...
  filter(!is.na(gonad),
         samp_grp == 1) %>%
  group_by(year, bed_code, district) %>%
  mutate(n_shucked = n()) %>%
  group_by(year, bed_code, district, samp_grp, gonad) %>%
  summarise(perc = n() / mean(n_shucked) * 100) %>%
  ...
  # write output table
  write_csv("./output/fishery_independent/statewide_scallop_survey/2021/2021_gonad_condition.csv")
